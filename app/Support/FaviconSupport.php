...
abstract class FaviconSupport
{
    ...
    {
        $manifest = config('nativekit.favicon.manifest');
...
<browserconfig>
    ...
    {
        return response(Vite::content(config('nativekit.logo')), 200, ['Content-Type' => 'image/png']);
    }

    private static function getFavicon(): Response
    {
        return response(Vite::content(config('nativekit.favicon')), 200, ['Content-Type' => 'image/x-icon']);
    }
}
